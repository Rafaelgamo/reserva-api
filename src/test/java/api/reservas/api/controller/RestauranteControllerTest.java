...
public class RestauranteControllerTest {
	...
	void setUp() {
		...
		Mockito.when(buscarRestaurantesUseCase.filtrarAbertosNoMomento(Mockito.any(PagingInfo.class)))
				.thenReturn(pagedResult);
	}

	@Test
	void deveListarRestaurantesAbertosAgora() throws Exception {
		mockMvc.perform(get("/restaurante").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
	}
}